...
const handleJoin = function (ctx, args) {
    ...
    var next = function (err, message, preUserListFunction) {
        ...
        if (err) {
            return void user.sendMsg([seq, 'ERROR', err, message]);
        }
        ...
        if (chan.id.length === ADMIN_CHANNEL_LENGTH) {
            // Join callback
            user.sendMsg([seq, 'JACK', chanName]);
            // Send HK id
            preUserListFunction();
            // Send your ID to complete the JOIN process
            return void user.sendMsg([0, user.id, 'JOIN', chanName]);
        }
        ...
        if (userIndex !== -1) {
            ...
            user.sendMsg([seq, 'ERROR', 'EJOINED', chanName]);
            ...
            chan.forEach(function (u) {
                if (u === user) { return; }
                user.sendMsg([0, u.id, 'JOIN', chanName]);
            });
...
const handleMsg = function (ctx, args) {
    ...
    if (obj && !ctx.channels[obj] && !ctx.users[obj]) {
        ...
        return void user.sendMsg([seq, 'ERROR', 'enoent', obj]);
    }
    ...
    if ((target = ctx.channels[obj])) {
        return void sendChannelMessage(ctx, target, json, function (err) {
            if (err) { return void user.sendMsg([seq, 'ERROR']); }
            user.sendMsg([seq, 'ACK']);
        });
    }
    ...
    if ((target = ctx.users[obj])) {
        json.unshift(0);
        return void target.sendMsg(json);
    }
};
...
const handleLeave = function (ctx, args) {
    ...
    user.sendMsg([seq, 'ERROR', err, obj]);
};
...
module.exports.create = function () {
    ...
    Server.send = function (userId, msg, cb) {
        ctx.users[userId].sendMsg(msg, cb);
    };
    ...
    Server.shutdown = function () {
        ...
        // XXX EMIT SHUTDOWN TO ALL WORKERS
    ...
    // XXX
    let onSessionOpen = (data, sendMsg) => {
        let user = {
            sendMsg,
            id: data.id,
            inQueue: 0,
    ...
    let onSessionClose = data => {
        let user = ctx.users[data.id];
        dropUser(ctx, user, 'SOCKET_CLOSED');
    };
    let onSessionMessage = data => {
        let cmd = data.type;
        let args = data.args;
        let user = ctx.users[data.args.id];
    ...
    Server.give = (type, data, sendMsg) => {
        if (!ctx.active) { return; }
        if (type === 'open') {
            return onSessionOpen(data, sendMsg);
        }
        if (type === 'close') {
            return onSessionClose(data);
        }
