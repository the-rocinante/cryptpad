...
var wss = new WebSocketServer({port:Env.websocketPort, path: "/cryptpad_websocket"});
...
let sendMain = type => {
    return data => {
        sendMessage({
            ...
            if (err) { console.error('sendMain error', err); }
        });
    };
};
let Server = Env.Server = Netflux.create(wss)
    .on('sessionClose', sendMain('close'))
    .on('sessionOpen', sendMain('open'))
    .on('message', sendMain('message'))
    .on('error', function (error, label, info) {
        console.error(error);
    });

EVENTS.WS = function (msg, cb) {
    let userId = msg.id;
    let data = msg.data;
    Server.send(userId, data, cb);
};
